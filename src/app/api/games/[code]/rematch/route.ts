...
import { rematch, GameError } from "@/lib/game/engine";
...
export async function POST(
  ...
  if (game.hostPlayerId !== me.id)
    return NextResponse.json({ error: "só o host pode reiniciar" }, { status: 403 });

  try {
    const result = await rematch(game.id);
    return NextResponse.json(result);
  } catch (e) {
    if (e instanceof GameError)
      return NextResponse.json({ error: e.message, code: e.code }, { status: 400 });
    console.error("[rematch route] failed:", e);
    return NextResponse.json(
      { error: "falha ao reiniciar partida", detail: String(e) },
      { status: 500 }
    );
  }
}
